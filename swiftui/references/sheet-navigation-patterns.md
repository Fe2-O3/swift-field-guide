...
### Navigation Coordinator (centralized, deep-link-ready)

For apps with non-trivial navigation logic, encapsulate the `NavigationPath` and navigation methods in an `@Observable` coordinator rather than manipulating `navigationPath` inline. This centralizes navigation logic, keeps it testable, and is deep-link ready:
...
Mutating `NavigationPath` incorrectly (e.g. removing more elements than exist, or mixing route types without a stable `Hashable` enum) can leave it in an inconsistent state — the coordinator's `pop()`/`navigateToRoot()` guard against that.
...
## Additional Navigation & Presentation Rules

- Flag any use of the deprecated `NavigationView` — use `NavigationStack` or `NavigationSplitView`.
- Prefer `navigationDestination(for:)` over the old `NavigationLink(destination:)` pattern.
...
- `navigationDestination(for:)` must be registered once per data type; flag duplicate registrations for the same type.
- Attach `confirmationDialog()` to the specific UI element that triggers it (not to a container further up the hierarchy) so Liquid Glass animations can originate from the correct source.
- If an alert has only a single "OK" button that does nothing but dismiss it, it can be omitted entirely: `.alert("Dismiss Me", isPresented: $isShowingAlert) { }`.
- If a sheet presents an optional piece of data, prefer `sheet(item:)` over `sheet(isPresented:)` so the optional is unwrapped safely.
- When the presented view's initializer takes only the item, prefer `sheet(item: $someItem, content: SomeView.init)` over `sheet(item: $someItem) { someItem in SomeView(item: someItem) }`.
