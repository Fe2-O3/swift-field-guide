...
### Flagged conflict — @ViewBuilder computed properties for composition

One merged source (`swiftui-patterns`) taught `@ViewBuilder` computed properties (`headerView`, `contentView`, `footerView`, `backgroundView`, etc.) as *the* recommended pattern for structuring conditional view content — with no performance caveat. That directly conflicts with the guidance above, and with a second source's stance ("it is more efficient to break views up by making dedicated SwiftUI views rather than place them into computed properties or methods — using `@ViewBuilder` on a property or method does not solve this"). Two of the three sources on this topic agree, and the mechanism above (the whole computed property re-executes on every parent redraw, unlike an extracted `struct View` whose `body` SwiftUI can skip) is a real, verifiable reason — so **prefer extracting to a separate `View` struct, in its own file, over a `@ViewBuilder` computed property**, especially once the fragment does anything non-trivial (loops, conditionals, image loading). Small private `@ViewBuilder` properties that are cheap, tightly coupled to `body`, and would read acceptably if inlined are a reasonable exception — reserve them for genuinely small fragments, not general-purpose decomposition.

### Additional view-structure rules

- Extract button actions and other logic out of `body` into separate methods or a view model; don't mix layout and business logic inline in `task()`, `onAppear()`, or button closures.
- Each type (`struct`/`class`/`enum`) belongs in its own Swift file — flag files defining multiple types.
- Prefer `TextField(_:text:axis: .vertical)` over `TextEditor` unless a full-screen editing experience is required — it supports placeholder text, which `TextEditor` does not. Pair with `.lineLimit(5...)` if a minimum height is needed.
- Prefer the direct-action initializer when available: `Button("Label", systemImage: "plus", action: myAction)` over `Button("Label", systemImage: "plus") { myAction() }`.
- When rendering SwiftUI views to images, prefer `ImageRenderer` over `UIGraphicsImageRenderer`.
- For `TabView(selection:)`, bind to an enum, not an `Int`/`String`: `Tab("Home", systemImage: "house", value: .home)` over `Tab("Home", systemImage: "house", value: 0)`.
...
For full `UIViewRepresentable`/`UIViewControllerRepresentable` worked examples (a `WKWebView` wrapper with a navigation-delegate `Coordinator`, and a `PHPickerViewController` image picker), see `references/uikit-interop.md`. Note also: when targeting iOS 26+, SwiftUI ships a native `WebView` type (`import WebKit`) that replaces most hand-wrapped `WKWebView` uses — reach for `UIViewRepresentable` only when no native equivalent exists.
