...
A flat list of further API-choice rules that don't fit the deprecated-to-modern version-segment format above but are worth enforcing during review:

- Prefer `overlay(alignment:content:)` over the deprecated `overlay(_:alignment:)` — use `.overlay { Text("Hello, world!") }` rather than `.overlay(Text("Hello, world!"))`.
- Never use `.navigationBarLeading`/`.navigationBarTrailing` for toolbar item placement — they're deprecated. Use `.topBarLeading`/`.topBarTrailing`.
- For automatic grammar agreement in English, French, German, Portuguese, Spanish, and Italian, use inflection: `Text("^[\(people) person](inflect: true)")` to show a pluralized count correctly.
- Shapes can be filled and stroked with two chained modifiers — no overlay needed for the stroke (fixed in iOS 17+; the overlay workaround was only required before that).
- When the project is configured for generated asset symbols, prefer `Image(.avatar)` over the string-literal `Image("avatar")`.
...
- To hide scroll indicators, use `.scrollIndicators(.hidden)` rather than `showsIndicators: false` in an initializer.
...
- If `ObservableObject` is genuinely required (e.g. wiring a Combine publisher into a debouncer), remember `import Combine` explicitly — it's no longer re-exported through `import SwiftUI`.
...
| `showsIndicators: false` (initializer) | `.scrollIndicators(.hidden)` | iOS 16+ |
| `Text` concatenation with `+` | `Text("\(textA)\(textB)")` interpolation | iOS 15+ |
