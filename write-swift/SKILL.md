...
- **Macros obey four rules.** Additive only — a macro can add code, never remove it. Deterministic — the same input always expands to the same output. Sandboxed — the expander has no file system, network, or environment access. Inspectable — every expansion is viewable with "Expand Macro".
- **The compiler can derive real boilerplate for you.** `@OptionSet` on a struct with a private options enum generates the `OptionSet` conformance, static members, and set algebra. `#URL("https://example.com")` parses and validates a URL literal at compile time.
- **Macro implementations live in a separate package** built on SwiftSyntax, not in the target that uses them. Using macros is routine; authoring them is advanced.
...
| Duplicate implementations per OS version                              | **`@backDeployed(before:)`** — one new implementation, back-deployed to older OS versions                | 5.8   |
| Blocking APIs that callers must not use from async contexts           | **`@available(*, noasync)`** — compile error on direct calls from async code                             | 5.7   |
| Re-declaring a dependency's API to expose it from your module         | **`@_exported import`** — re-exports a module's public API (underscore attribute; unstable, use sparingly) | —     |
...
Moving a component from UIKit to SwiftUI is mechanical once you map the vocabulary. SwiftUI inverts the control flow — state drives the view — so port what the view shows, not how it laid out.
...
The delegate callbacks a UIKit view needs (table data sources, navigation delegates) mostly disappear: `List` and `NavigationStack` derive behavior from your data. Budget a full rework per view controller rather than a line-by-line port.
