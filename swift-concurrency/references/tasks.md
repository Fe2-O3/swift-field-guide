...
- What is a Task?
- Cancellation (incl. `withTaskCancellationHandler`, broken cancellation patterns)
- Task Groups (incl. limiting concurrency, partial-result error handling)
- Discarding Task Groups
- Advanced: Task Timeout Pattern
- SwiftUI Integration
- Structured vs Unstructured Tasks (incl. when `Task {}` is a code smell)
- Task Priorities (incl. priority escalation API)
...
### Bridging cancellation to legacy APIs: `withTaskCancellationHandler`

Cancellation is cooperative — setting the cancelled flag does nothing unless running code checks it. `withTaskCancellationHandler` bridges Swift's cooperative cancellation to legacy APIs that expose their own cancel mechanism. The `onCancel` closure fires immediately when cancellation is requested — even while the async body is suspended — and may run on any thread.
...
This is most useful when wrapping something that gives you a cancel handle. If the wrapped async call already checks task cancellation internally (as most `URLSession` APIs do), `onCancel` has nothing to do.

### Broken cancellation patterns

**Catching and ignoring `CancellationError`** — retrying or showing an alert for a normal lifecycle event:
...
// FIXED: Let cancellation pass through silently.
...
**Forgetting to cancel stored tasks** — the task keeps running after the object is done with it:
...
// FIXED: cancel the previous task before starting a new one, and on teardown.
func load() {
    loadTask?.cancel()
    loadTask = Task { await fetchData() }
}
...
**No cancellation checks in CPU-bound work** — a tight computational loop with no `await` runs to completion even if cancelled, because there's no suspension point where cancellation can take effect. Insert periodic `try Task.checkCancellation()` calls wherever it's safe to bail out.
...
Task groups launch all child tasks eagerly, which may be undesirable for things like large URL batches. Start a fixed batch and top it up as each child finishes:
...
By default, one child task throwing cancels the whole group. If you need partial results instead, catch errors inside each child task and report success/failure per item:
...
Swift can usually infer a task group's element type, but not always. Simple types like `String`, `URL`, or `Data` typically work fine, but a tuple result like `(URL, Result<Data, Error>)` above requires the explicit `withTaskGroup(of: (URL, Result<Data, Error>).self)` — Swift cannot infer that on its own.
...
Creating a `Task {}` to call an async function from a synchronous context is sometimes necessary (e.g., a button action). But watch for these anti-patterns during review:

- **`Task` inside `onAppear()`**: never create a `Task` inside a SwiftUI `onAppear()`. Use the `.task()` modifier instead — it handles cancellation on disappear automatically (see SwiftUI Integration above).
- **`Task` bridging sync → async in a function that could itself be `async`**: if the caller can be made `async`, do that instead of wrapping in `Task {}`.
- **Ignoring the return value of a throwing task**: the error is silently lost. At minimum, handle the error inside the task closure (see Common Mistakes Agents Make below).
...
Swift 6.2 exposes this directly: tasks can observe escalation, and code can request a higher priority when needed.
...
Manual escalation exists, but most code should leave this to the runtime — reach for it only when a codebase is doing explicit advanced coordination, not for everyday task usage.
...
That ordering lets `Task.immediate` perform initial synchronous work before the caller continues, up to the first suspension point. Use it only when that immediate start is the point — it's still an unstructured task after the first synchronous stretch, and for most cases regular `Task {}` is still the right choice. Task groups also gained `addImmediateTask()` and `addImmediateTaskUnlessCancelled()` for the same immediate-start behavior with child tasks.

## Task naming (Swift 6.2)

Tasks and task-group children can carry names, which helps when one task misbehaves and you need to identify it in logs or traces. Names are a debugging aid, not a correctness feature — worth keeping when logs, tracing, or failure diagnosis matter.
...
- Swallowing errors in a throwing `Task {}` closure — `Task { try await riskyWork() }` silently drops the error if nothing inside the closure catches it. Handle it there (alert, log, `@State` error property) instead of letting it vanish.
