...
A `@concurrent` method on a `@MainActor` class cannot touch the class's own state — the method has left the main actor, so its cache is off-limits:
...
**Fix: check the cache at the caller**, and offload only the decode work:
...
`DispatchGroup.enter()`/`leave()` with `notify` maps to a task group: `withTaskGroup(of:)` + `group.addTask` for each unit of work, then await the group (iterate it, or `waitForAll()`) instead of registering a completion callback. Awaiting the group keeps the fan-out structured — child tasks can't outlive the scope, and results are collected in the loop where you consume them.
