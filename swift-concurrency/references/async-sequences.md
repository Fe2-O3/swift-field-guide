...
- AsyncStream / AsyncThrowingStream (incl. `makeStream(of:)` factory)
...
### Prefer the `makeStream(of:)` factory

The closure-based initializer above still works, but the modern way to create an `AsyncStream` is the static factory method, which returns the stream and its continuation as a tuple — no closure capture needed:

```swift
// OLD: Closure-based, awkward to store the continuation outside the closure.
var continuation: AsyncStream<Event>.Continuation?
let stream = AsyncStream<Event> { cont in
    continuation = cont
}

// NEW: Clean, continuation available immediately.
let (stream, continuation) = AsyncStream.makeStream(of: Event.self)
```

This also works with `AsyncThrowingStream.makeStream(of:throwing:)`. Prefer this form whenever the continuation needs to be stored on a property so something else (a delegate callback, a monitor, etc.) can yield into it later — see `migration.md#wrapping-delegate-based-apis`.

A continuation must always be finished exactly once. Failing to finish it causes the consumer's `for await` loop to hang indefinitely. Finishing it twice is a programmer error, though `AsyncStream.Continuation` tolerates it (unlike `CheckedContinuation`, which traps). Always finish in cleanup paths, and use `onTermination` when the producer can be torn down before it naturally completes:
