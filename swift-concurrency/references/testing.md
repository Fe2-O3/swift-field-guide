...
- Setup and Teardown
- Handling Flaky Tests (incl. `.serialized` parameterized-only caveat)
...
**Critical**: Must `await` async work for confirmation to validate. All async work being confirmed must also *complete* before the `confirmation()` closure returns — if the code under test spawns a `Task` internally and the test has no way to await that task, `confirmation()` finishes before the work does and the test fails. Either make the production API `async` so the test can await it directly, or have it return its `Task` handle so the test can `await task.value` before the closure ends.
...
**`.serialized` only affects parameterized tests.** It tells Swift Testing to run that test's argument cases one at a time rather than in parallel. Applying it to a non-parameterized test does nothing, and applying it to a whole suite only serializes the parameterized tests inside that suite — other tests in the suite are unaffected. Agents frequently assume `.serialized` works on any test; it does not.
...
**The goal is to verify the *code under test* checks for cancellation** — not just that `Task.checkCancellation()` works in a test harness. Design the test so the code under test is the thing observing the cancellation flag: feed it enough work that a cancellation check will be hit mid-flight, cancel while it's suspended, then assert it exits with `CancellationError`.
...
If the code under test is a `for await` loop, cancel the consuming task and verify the loop exits. The key point is that the test must exercise a cancellation check that lives in production code, not one added just for the test.
...
Enable Thread Sanitizer (TSan) in your test scheme to catch data races at runtime. TSan finds races that the compiler's static checks often miss, particularly in code using `@unchecked Sendable` or unsafe pointers.
