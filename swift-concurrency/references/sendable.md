...
**Use as last resort.** Tells compiler to skip verification—you guarantee thread-safety. Before reaching for it, check whether Swift 6's region-based isolation (below) already solves the problem — many cases that previously required `@unchecked Sendable` now compile cleanly without it.

The only broadly legitimate uses are: a type with internal locking (`os_unfair_lock`, `NSLock`, `Mutex`, etc.) that is genuinely thread-safe, or a reference type whose mutable state is protected by an actor in practice but can't express that to the compiler for some reason. A common red flag is applying it to silence a compiler error without understanding why the error exists — this was previously a Fix-It suggestion in Xcode, so it isn't rare — or applying it to a class with mutable `var` properties and no synchronization at all.
...
|----------|-------------------------|
| External type not Sendable (e.g. `CLLocation`) | ✅ Yes, with a TODO to revisit |
| Apple framework type not Sendable | ✅ Yes, with a TODO to revisit |
| Your own mutable class | ❌ No, make it an actor |
| Immutable reference type | ✅ Yes, if truly immutable |
| Type with `var` properties | ❌ No, use an actor or redesign |

Common types that force it because they aren't marked Sendable:

- `CLLocation`, `CLLocationCoordinate2D` (CoreLocation)
- Types from legacy Objective-C frameworks not yet audited for Sendable
...
Static sorting comparators are the common case where `nonisolated(unsafe) static var` looks necessary but isn't. Prefer `static let` with `@Sendable` closures — evaluated once, immutable, and provably free of mutable captures:
