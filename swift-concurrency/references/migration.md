...
- Deprecated API Replacements (keyWindow, device name, URLSession.dataTask)
- Continuations and Delegate Bridging (resume-once rule, delegate → AsyncStream, delegate value capture, serial queue → actor, locks vs actors)
...
While migrating, swap these legacy calls for their modern replacements:

| Deprecated | Modern Replacement |
|-----------|-------------------|
| `UIApplication.shared.keyWindow` | Key-window lookup over `UIApplication.shared.connectedScenes` |
| `UIDevice.current.name` | Requires a privacy manifest — it returns the user's device name |
| `URLSession.dataTask(with:completion:)` | `URLSession.data(from:)` async |

The `dataTask` row is the same rewrite shown in "Rewriting Closures to Async/Await" above.

---

## Continuations and Delegate Bridging

### Checked continuations: resume exactly once
...
- Resuming twice: a runtime crash (`CheckedContinuation` traps; the unsafe variant is undefined behavior).

Audit every code path — including the case where the callback might never fire because the object was deallocated — to make sure the continuation is still resumed.

Default to `withCheckedContinuation` / `withCheckedThrowingContinuation` everywhere, including production builds. The runtime checks catch double-resume and missing-resume bugs that are otherwise extremely hard to diagnose. Only consider the `withUnsafe...` variants after profiling proves the checked version is a bottleneck in a hot path — rare in practice.
...
For delegate patterns that deliver multiple values over time, use `AsyncStream` rather than a continuation (which only resumes once). Use `AsyncStream.makeStream(of:)` to get the stream and continuation as a pair — see `async-sequences.md#asyncstream` — and make sure:
...
This pattern supports a single consumer. For multiple consumers, broadcast through an `@Observable` class instead, or see `async-algorithms.md` for `AsyncChannel`.

Single-shot delegates (one callback, then done) can use `withCheckedContinuation` instead of a stream.

When a `nonisolated` delegate method must hand a value to main-actor state, extract what you need from the parameter **before** opening the task:

```swift
nonisolated func delegate(_ param: SomeType) {
    let value = param.value  // Step 1: Capture BEFORE the task
    Task { @MainActor in
...
Values read inside the task race with the delegate's own execution, and non-Sendable parameters can't cross into the `@MainActor` closure at all.

### Runtime actor assertions in callback code

Callback-based APIs are a common place for actor assumptions to fail at runtime. If a callback reaches main-actor state without carrying that guarantee in the type system, Swift 6 runtime checks can trap instead of silently racing. Use `MainActor.assumeIsolated()` (see `actors.md#mainactorassumeisolated`) only when the callback really is main-actor-bound and you're encoding a guarantee the compiler can't see.

### Serial `DispatchQueue` → `actor`

A serial dispatch queue protecting mutable state maps directly onto an `actor` (see `threading.md#gcd-to-isolation-domain-migration` for the one-line version of this mapping and the other GCD equivalents):
...
If the API must stay synchronous, prefer a lock over introducing actor isolation just to serialize access — an `actor` forces every access point to become `async`. `Mutex` (see `actors.md#mutex-alternative-to-actors`) gives the best compile-time story and can preserve checked `Sendable` on the owning type; traditional locks still work but the owning reference type often ends up needing `@unchecked Sendable`. Choose an actor only when the API itself should become actor-isolated, not merely as a way to get a lock.
...
| `CurrentValueSubject` | No direct equivalent — see Observation Alternative below |
| `publisher.values` | Already an `AsyncSequence` — use directly, don't re-wrap it |

Combine is not officially deprecated, but Apple's guidance is to avoid using it for new code. If a Combine publisher already exposes a `.values` property, consume that directly rather than wrapping it in a new `AsyncStream`.
