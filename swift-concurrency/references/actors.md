...
- Actor Isolation
- Global Actors / @MainActor (incl. `assertIsolated`)
- Isolated vs Nonisolated
- Actor Reentrancy (incl. deduplicating concurrent calls)
- Isolated Deinit / Isolated Conformances (Swift 6.2+)
- `#isolation` Macro
- Mutex: Alternative to Actors
- What introducing a custom actor changes (review checklist)
- Decision Tree
...
### assertIsolated for debugging

Global actors also have an `assertIsolated()` method, useful for catching isolation bugs during development: it halts debug builds if the current task isn't executing on the actor's serial executor.
...
`assertIsolated()` only operates in debug builds — like regular assertions, it's compiled out of release builds, so it has no shipping performance cost.
...
### Deduplicating concurrent reentrant calls

Capturing the result before assigning (above) stops the crash, but two callers can still both see a cache miss and both trigger a download. To dedupe concurrent callers, store in-flight tasks keyed by the same identity:
...
A custom actor introduces a separate serialized access boundary. When reviewing new actor types, check:

- External callers must use `await` to reach it.
- Values crossing the boundary must satisfy `Sendable`.
- Reentrancy rules apply after every suspension point inside the actor (see Actor Reentrancy above).

Flag actor types whose API mostly forwards work or owns little mutable state — that's often a sign a plain type or a `Mutex` would have been simpler. Actors aren't always the default answer; don't reach for one just because state needs protecting. See Matt Massicotte's writing on actors for further reading: <https://www.massicotte.org/actors/>.
