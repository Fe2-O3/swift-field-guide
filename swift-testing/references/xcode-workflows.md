...
(See `references/traits-and-tags.md` for another set of suggested tags — `.networking`, `.slow`, `.edgeCase`, `.smoke` — geared toward domain grouping rather than test-plan filtering; the two vocabularies are complementary, not competing.)

## Test plans

- Configure include/exclude tags per target in test plans.
- Use "any tags" vs "all tags" intentionally when combining filters.
- Maintain separate plans for: fast core checks, integration checks, slower/optional scenarios.
...
- Improve argument/type descriptions for faster root-cause identification (see `CustomTestStringConvertible` in `references/expectations.md`).
