...
- Random, parallel execution is the standard, not an opt-in — write every test assuming it can run in any order, at any time, alongside any other test.
...
Note that a fresh test *instance* is created per test — but `static` properties and singletons are not instance state, so they still leak across tests even though "each test gets a new instance" sounds like it should prevent that.
...
- Isolate state per test invocation (fresh suite instance helps for instance properties, not for `static`/singleton state).
...
- `.serialized` behaves differently depending on whether it's applied to a suite or to a single parameterized test's cases — see `references/async-testing.md` for the exact distinction before reaching for it.
...
- use in-memory substitutes (see `references/writing-better-tests.md`)
- create a separate serial test plan for the integration path (see `references/xcode-workflows.md`)

Prefer an architecture that supports both fast in-memory tests and selective real integration tests.
