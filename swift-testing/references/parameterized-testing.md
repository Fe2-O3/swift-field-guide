...
- Replace copy-pasted tests and in-test `for` loops with `@Test(arguments: ...)` (loops give worse diagnostics — one failing argument doesn't isolate itself).
...
- Individual failing arguments can be rerun without rerunning all inputs (see `references/xcode-workflows.md`).
...
- Two collections generate all combinations (Cartesian product) — this is the single most common Swift Testing surprise. `@Test(arguments: [a, b], [c, d])` creates 4 combinations, not 2.
- Control explosion by: reducing argument sets, splitting tests by concern, or pairing related values via `zip(...)`.
...
- Ensure argument types are readable in output; provide a custom test description if noisy (see `CustomTestStringConvertible` in `references/expectations.md`).
- Keep argument lists easy to scan (multi-line formatting is recommended).
- Avoid extracting argument arrays into separate properties or extensions — this hides what the test covers and forces readers to jump between definitions. Keep arguments inline unless the list is genuinely reused across multiple test functions.
...
- **Extracting argument arrays into separate properties or extensions**, hiding what's covered.
